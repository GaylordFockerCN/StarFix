...
public class SeeThroughSkill extends Skill {
    ...
    public void onInitiate(SkillContainer container) {
        ...
        container.getExecuter().getEventListener().addEventListener(EventType.HURT_EVENT_PRE, EVENT_UUID, (event) -> {
            LivingEntityPatch<?> ep = (LivingEntityPatch)EpicFightCapabilities.getEntityPatch(((DamageSource)event.getDamageSource()).getEntity(), LivingEntityPatch.class);
            if (ep != null && event.isParried()) {
                DynamicAnimation patt2326$temp = ep.getAnimator().getPlayerFor((DynamicAnimation)null).getAnimation();
                if (patt2326$temp instanceof StaticAnimation) {
                    ...
                    if (Guard_Array.isNoParry(animation)) {
                        ((Player)container.getExecuter().getOriginal()).addEffect(new MobEffectInstance(MobEffects.GLOWING, 60, 60, false, false));
        ...
        container.getExecuter().getEventListener().addEventListener(EventType.DODGE_SUCCESS_EVENT, EVENT_UUID, (event) -> {
            ...
            if (ep != null) {
                DynamicAnimation patt3283$temp = ep.getAnimator().getPlayerFor((DynamicAnimation)null).getAnimation();
    ...
    public void updateContainer(SkillContainer container) {
        ...
        if ((Integer)container.getDataManager().getDataValue(ACTIVE_TIME) >= 60) {
            container.getDataManager().setData(CUT, false);
            container.getDataManager().setData(LEG, false);
            ((Player)container.getExecuter().getOriginal()).removeEffect(MobEffects.GLOWING);
        }

        if ((Integer)container.getDataManager().getDataValue(ACTIVE_TIME) < 60) {
            container.getDataManager().setData(ACTIVE_TIME, (Integer)container.getDataManager().getDataValue(ACTIVE_TIME) + 1);
        }

        boolean isCut = (Boolean)container.getDataManager().getDataValue(CUT);
        boolean isLeg = (Boolean)container.getDataManager().getDataValue(LEG);
        if (isLeg && container.getDataManager().getDataValue(IS_CUT_DOWN) && !(container.getExecuter().getAnimator().getPlayerFor((DynamicAnimation)null).getAnimation() instanceof AttackAnimation)) {
        ...
        if (isCut && container.getDataManager().getDataValue(IS_CUT_DOWN) && !(container.getExecuter().getAnimator().getPlayerFor((DynamicAnimation)null).getAnimation() instanceof AttackAnimation)) {
